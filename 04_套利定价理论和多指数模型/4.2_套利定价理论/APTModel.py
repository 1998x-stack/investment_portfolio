...
class APTModel:
    """
    Arbitrage Pricing Theory (APT) model class to estimate expected returns based on multiple risk factors.
    ...
    def __init__(self, risk_free_rate: float, factor_names: List[str]):
        """
        Initialize the APTModel with a risk-free rate and names of the risk factors.
...
apt_model = APTModel(risk_free_rate, factor_names)

# 加载历史数据
apt_model.load_data(historical_data)

# 估计参数
lambda_est, beta = apt_model.estimate_parameters()
print(f"Risk Premiums (Lambda): {lambda_est}")
print(f"Factor Sensitivities (Beta): {beta}")

# 预测回报率
factor_values = np.array([0.02, 0.03])
expected_return = apt_model.predict_return(factor_values)
print(f"Predicted Expected Return: {expected_return}")

# 评估模型
r_squared = apt_model.evaluate_model()
...
print("All checks passed. The APT model is correctly implemented and evaluated.")
